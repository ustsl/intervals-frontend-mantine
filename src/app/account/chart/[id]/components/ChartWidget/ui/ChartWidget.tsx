...
export const ChartWidget = ({ id }: { id: string }) => {
    ...
    const availableFields =
        data && data.data_relation && data.data_relation.container && data.data_relation.container.length > 0
            ? Object.keys(data.data_relation.container[0]).map(key => ({ value: key, label: key }))
            : [];
    ...
    return (
        <ObjectWrapper error={error} loading={loading}>
            ...
            <Grid>
                ...
                <Grid.Col span={7}>
                    <ChartItemEntity
                        data={data.data_relation?.container || []}
                        settings={editedSettings}
                    />
                </Grid.Col>
            </Grid>
